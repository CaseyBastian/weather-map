...
const forecastLayersArr: ForecastLayer[] = [
	...
	{ name: 'Phoenix', visible: true, latitude: 33.4484, longitude: -112.074 },
	{
		name: 'New Orleans',
		visible: true,
		latitude: 29.9511,
...
export class WeatherLayersService {
	...
	getVisibleLayers(
		...
		const values =
			SourceType === SourceLayerType.EVENT
				? this.eventLayersSource.value
				: this.forecastLayersSource.value;
	...
	async fetchForecastData(gridPoint: {
		gridId: string;
		gridX: number;
		gridY: number;
	...
	async fetchHourlyForecastData(gridPoint: {
		gridId: string;
		gridX: number;
		gridY: number;
	}): Promise<any> {
